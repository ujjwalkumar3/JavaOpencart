...
public class FDCalculator {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		//run loop till no. of rows
		for(int i=1;i<=rows;i++)
		{
			//1) read data from excel
			String pric=ExcelUtils.getCellData(filePath, "Sheet1",i,0); //cell no. 0 in first row
			...
			driver.findElement(By.xpath("//input[@id='tenure']")).sendKeys(per1); //period 1
			
			Select perdrp=new Select(driver.findElement(By.xpath("//select[@id='tenurePeriod']")));
			perdrp.selectByVisibleText(per2); //period 2 year
			
			
			Select fredrp=new Select(driver.findElement(By.xpath("//select[@id='frequency']"))); //frequency
			...
			Thread.sleep(3000);
			//after validation click on clear for next set of test
